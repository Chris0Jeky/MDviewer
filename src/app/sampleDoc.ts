/**
 * Bundled demo document loaded by the "Try a sample document" link in the empty
 * state. It deliberately exercises every renderer seam MDviewer supports —
 * headings, prose, an auto table of contents, fenced code in several languages,
 * inline and display KaTeX, a Mermaid flowchart, all four callout types,
 * footnotes, a table, a task list, and one intentionally over-tall code block so
 * the no-slice page-break guarantee is visible in the very first thing a user sees.
 *
 * IMPORTANT: the body of this string is kept byte-identical to
 * `tests/fixtures/sample.md`. A unit test asserts the two stay in sync, so any
 * edit here must be mirrored there (and vice versa).
 */

export const SAMPLE_MARKDOWN = `# MDviewer — Sample Document
